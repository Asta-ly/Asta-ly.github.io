...
{         	
	...
	if(num < 0)
	{
		OLED_ShowChar(x-size2/2,y,'-',size2);
		num = -num;
	}
	else
		OLED_ShowChar(x-size2/2,y,' ',size2);
...
void oled_show_char(uint8_t x, uint8_t y, uint8_t num, uint8_t size)
{
    ...
    for(j = 0; j < page; j++)
    {
        ...
        {
            if(size == 12)
                Write_IIC_Data(ascii_6X12[num][i]);
            else if(size == 16)
                Write_IIC_Data(ascii_8X16[num][i]);
            else if(size == 24)
                Write_IIC_Data(ascii_12X24[num][i]);
...
void oled_show_chinese(uint8_t x, uint8_t y, uint8_t N, uint8_t message_type)
{
    ...
    for(j = 0; j < 2; j++)
    {
        ...
        for(i = 16 * j; i < 16 * (j + 1); i++)
        {
            ...
            {
                case SHOW_INPUT_PWD:
                    Write_IIC_Data(chinese_enter_password[N][i]);
                    break;
                
                case SHOW_PWD_RIGHT:
                    Write_IIC_Data(chinese_password_right[N][i]);
                    break;
                
                case SHOW_PWD_WRONG:
                    Write_IIC_Data(chinese_password_wrong[N][i]);
                    break;
                
                case SHOW_INPUT_OLD_PWD:
                    Write_IIC_Data(chinese_enter_old_password[N][i]);
                    break;
                
                case SHOW_INPUT_NEW_PWD:
                    Write_IIC_Data(chinese_enter_new_password[N][i]);
                    break;
                
                case SHOW_PWD_CHANGED:
                    Write_IIC_Data(chinese_password_changed[N][i]);
                    break;
                
                case SHOW_SET_PWD:
                    Write_IIC_Data(chinese_set_password[N][i]);
                    break;
